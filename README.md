...
### Caveats
 ...
 - To build `libsarus` as a shared library, add `-DBUILD_SHARED_LIBS=TRUE` to CMake options in `./build.sh`. (default: `FALSE`)
    - Warning: the shared `libsarus` additionally requires Boost 1.85 (`filesystem` and `regex`) at runtime.
