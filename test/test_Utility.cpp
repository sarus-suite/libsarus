...
TEST_F(UtilityTest, getEnvironmentVariable) {
...
TEST_F(UtilityTest, setEnvironmentVariable) {
  ...
  if (unsetenv(testKey.c_str()) != 0) {
    auto message =
        boost::format("Error un-setting the variable used by the test: %s") %
        strerror(errno);
  ...
  if (envValue == nullptr) {
    auto message =
        boost::format("Error getting the test variable from the environment");
    FAIL() << message.str().c_str();
  }
...
TEST_F(UtilityTest, parseKeyValuePair) {
...
TEST_F(UtilityTest, switchIdentity) {
  ...
  libsarus::filesystem::createFileIfNecessary(testDirRAII.getPath() / "file", 0,
                                              0);
...
TEST_F(UtilityTest, setFilesystemUid) {
...
TEST_F(UtilityTest, makeUniquePathWithRandomSuffix) {
  ...
  EXPECT_TRUE(
      boost::regex_match(uniquePath.string().c_str(), matches, expectedRegex));
}
...
TEST_F(UtilityTest, createFileIfNecessary) {
...
TEST_F(UtilityTest, copyFile) {
  ...
  EXPECT_EQ(libsarus::filesystem::getOwner(testDir / "non-existing-folder"),
            (std::tuple<uid_t, gid_t>{1000, 1000}));
  EXPECT_EQ(libsarus::filesystem::getOwner(testDir / "non-existing-folder/dst"),
            (std::tuple<uid_t, gid_t>{1000, 1000}));

  boost::filesystem::remove_all(testDir);
}

TEST_F(UtilityTest, copyFolder) {
...
TEST_F(UtilityTest, countFilesInDirectory) {
  // nominal usage
  {
...
TEST_F(UtilityTest, realpathWithinRootfs) {
  ...
  EXPECT_EQ(
      symlink("../../dir0/dir1/link_relative/dir2/dir3",
  ...
  EXPECT_EQ(
      symlink("/dir0/dir1/link_absolute/dir2/dir3",
  ...
  EXPECT_EQ(
      symlink(
          "/dir0/dir1",
          (rootfs / "dirX/link_absolute_with_no_common_path").string().c_str()),
      0);
  EXPECT_EQ(libsarus::filesystem::realpathWithinRootfs(
...
TEST_F(UtilityTest, resolveSharedLibAbi) {
  ...
  EXPECT_EQ(libsarus::sharedlibs::resolveAbi(testDir / "libtest_symlink.so.1"),
            (std::vector<std::string>{"1", "2"}));
  ...
  // For these cases, we trust the vendor and resolve the Lib Abi to that of the
  // symlink. Note here we use libtest.so.1 as the "original lib file" and
...
TEST_F(UtilityTest, getSharedLibSoname) {
  ...
  EXPECT_THROW(libsarus::sharedlibs::getSoname(dummyLibsDir / "lib_dummy_0.so",
                                               "readelf"),
               libsarus::Error);
}
...
TEST_F(UtilityTest, serializeJSON) {
  ...
  auto expected =
      std::string{"{\"string\":\"stringValue\",\"int\":11,\"array\":[0,1,2]}"};
...
TEST_F(UtilityTest, getCpuAffinity_setCpuAffinity) {
  ...
  if (initialCpus.size() <= 1) {
    std::cerr << "Skipping CPU affinity unit test. Not enough CPUs available"
              << std::endl;
    return;
  }
