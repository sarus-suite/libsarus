...
TEST_F(MountTest, mount_test) {
  ...
  auto sourceFile =
      libsarus::PathRAII{boost::filesystem::path{"./user_mounts_source_file"}};
  ...
  EXPECT_EQ(WIFEXITED(ret) != 0 && WEXITSTATUS(ret), 0);
  ...
  // test mount of existing destination directory
  {
    ...
    libsarus::Mount{sourceDir, destinationDir.c_str(), mount_flags, rootfsDir,
                    userIdentity}
        .performMount();
    EXPECT_TRUE(aux::filesystem::areDirectoriesEqual(
        sourceDir.string(), (rootfsDir / destinationDir).string(), 1));
