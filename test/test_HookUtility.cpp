...
TEST_F(HooksUtilityTest, DISABLED_findSubsystemMountPaths) {
    ...
    // single line corresponding to searched entry
    {
        ...
        auto expectedMountPoint =
            boost::filesystem::path("/sys/fs/cgroup/devices");
        ...
        libsarus::filesystem::writeTextFile(mountinfoContent.str(),
                                            mountinfoPath, openmode);
        ...
        EXPECT_TRUE(boost::filesystem::equivalent(returnedMountRoot,
                                                  expectedMountRoot));
        EXPECT_TRUE(boost::filesystem::equivalent(returnedMountPoint,
                                                  expectedMountPoint));
    }
    ...
    // multiple lines with several fs types
    {
        ...
        auto mountinfoContent =
            boost::format(
                "18 41 0:17 / /sys rw,nosuid,nodev,noexec,relatime "
    ...
    // malformed line on another entry (/proc missing superOptions and
    // filesystem type)
    {
        auto expectedMountRoot = boost::filesystem::path("/");
        auto expectedMountPoint =
...
TEST_F(HooksUtilityTest, findCgroupPathInHierarchy) {
    ...
    // single line corresponding to searched entry
    {
        ...
        libsarus::filesystem::writeTextFile(procFileContent.str(), procFilePath,
                                            openmode);
        auto returnedPath = findCgroupPathInHierarchy(
            "devices", testDir.getPath(), subsystemMountRoot, 1);
    ...
    // subsystem mount root is not filesystem root but not a prefix of cgroup
    // path
    {
    ...
    // thus a cgroup version 2 line will always be at the bottom of the list on
    // a real cgroup file.
    {
        auto subsystemMountRoot = boost::filesystem::path("/");
        auto expectedPath = boost::filesystem::path("/user.slice");
...
TEST_F(HooksUtilityTest, parseLibcVersionFromLddOutput) {
    EXPECT_EQ(
        (std::tuple<unsigned int, unsigned int>{2, 34}),
    ...
    EXPECT_EQ((std::tuple<unsigned int, unsigned int>{2, 31}),
              parseLibcVersionFromLddOutput(
