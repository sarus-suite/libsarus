...
static bool lockAcquisitionDoesntThrow(
    const boost::filesystem::path &fileToLock,
...
TEST_F(FlockTest, lock_is_released_when_the_object_is_destroyed) {
    ...
    {
        EXPECT_TRUE(lockAcquisitionDoesntThrow(
            fileToLock, libsarus::Flock::Type::writeLock));
    }
    {
        EXPECT_TRUE(lockAcquisitionDoesntThrow(
            fileToLock, libsarus::Flock::Type::writeLock));
...
TEST_F(FlockTest, move_constructor_moves_resources) {
    libsarus::Flock original{fileToLock, libsarus::Flock::Type::writeLock};
    {
        ...
        EXPECT_THROW(libsarus::Flock(fileToLock,
                                     libsarus::Flock::Type::writeLock, 10_ms),
                     libsarus::Error);
    }
