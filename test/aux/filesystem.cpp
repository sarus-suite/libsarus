...
bool areDirectoriesEqual(const std::string &dir1, const std::string &dir2,
                         bool compare_file_attributes) {
  ...
  for (int i = 0; i < n1 && ret; ++i) {
    ...
    if (S_ISDIR(statData1.st_mode) && filename1 != ".." && filename1 != ".") {
    ...
    if (compare_file_attributes && filename1 != ".." && filename1 != ".") {
      bool mode_check =
          ((statData1.st_mode & 0000777) != (statData2.st_mode & 0000777));
  ...
  for (uint i = 0; i < subdirs1.size() && ret; ++i) {
    if (!areDirectoriesEqual(subdirs1[i], subdirs2[i],
                             compare_file_attributes)) {
      ret = false;
    }
  }
...
static std::tuple<dev_t, ino_t>
getDeviceIDAndINodeNumber(const boost::filesystem::path &path) {
  struct stat sb;
...
bool isSameBindMountedFile(const boost::filesystem::path &file0,
...
static void createFilesystemNode(const boost::filesystem::path &path,
                                 const mode_t mode, const dev_t deviceID) {
  if (mknod(path.c_str(), mode, deviceID) != 0) {
    auto message = boost::format("Failed to mknod test device file %s: %s") %
                   path % strerror(errno);
...
void createCharacterDeviceFile(const boost::filesystem::path &path,
                               const int majorID, const int minorID,
                               const mode_t mode) {
