...
bool areDirectoriesEqual(const std::string &dir1, const std::string &dir2,
    ...
    for (int i = 0; i < n1 && ret; ++i) {
        ...
        if (S_ISDIR(statData1.st_mode) && filename1 != ".." &&
            filename1 != ".") {
...
static std::tuple<dev_t, ino_t> getDeviceIDAndINodeNumber(
...
static void createFilesystemNode(const boost::filesystem::path &path,
    ...
    if (mknod(path.c_str(), mode, deviceID) != 0) {
        auto message =
            boost::format("Failed to mknod test device file %s: %s") % path %
            strerror(errno);
...
void createTestDirectoryTree(const std::string &dir) {
    ...
    libsarus::process::executeCommand("chmod 665 " + dir +
                                      "/sub1/ssub11/g.pdf");
