...
DeviceParser::parseDeviceRequest(const std::string &requestString) const {
  ...
  if (requestVector.size() > 3) {
    auto message =
        boost::format("Invalid device request '%s': too many tokens provided. "
  ...
  try {
    ...
    auto baseMount =
        libsarus::Mount{source, destination, flags, rootfsDir, userIdentity};
...
DeviceParser::createDeviceAccess(const std::string &accessString) const {
  try {
    ...
    auto message =
        boost::format("%s. "
                      "Device access must be entered as a combination of 'rwm' "
                      "characters, with no repetitions") %
        e.what();
    SARUS_RETHROW_ERROR(e, message.str(), libsarus::LogLevel::INFO);
  }
}
