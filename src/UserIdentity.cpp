...
UserIdentity::UserIdentity() {
    ...
    if (numOfSupplementaryGids == -1) {
        auto message =
            boost::format("Failed to getgroups: %s") % strerror(errno);
        SARUS_THROW_ERROR(message.str());
    }

    if (numOfSupplementaryGids > 0) {
        supplementaryGids = std::vector<gid_t>(numOfSupplementaryGids);
        if (getgroups(supplementaryGids.size(), supplementaryGids.data()) ==
            -1) {
            auto message =
                boost::format("Failed to getgroups: %s") % strerror(errno);
            SARUS_THROW_ERROR(message.str());
...
UserIdentity::UserIdentity(uid_t uid, gid_t gid,
                           const std::vector<gid_t> &supplementaryGids)
    : uid{uid}, gid{gid}, supplementaryGids(supplementaryGids) {}
