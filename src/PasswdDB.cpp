...
void PasswdDB::write(const boost::filesystem::path &file) const {
  ...
  for (const auto &entry : entries) {
    os << entry.loginName << ":" << entry.encryptedPassword << ":" << entry.uid
       << ":" << entry.gid << ":" << entry.userNameOrCommentField << ":"
...
boost::filesystem::path PasswdDB::getHomeDirectory(uid_t uid) const {
  for (const auto &entry : entries) {
    if (entry.uid == uid) {
      logMessage(boost::format("Found home directory for uid=%d: %s") % uid %
                     entry.userHomeDirectory,
