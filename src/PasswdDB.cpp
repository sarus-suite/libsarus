...
PasswdDB::PasswdDB(std::istream &is) { read(is); }

void PasswdDB::write(const boost::filesystem::path &file) const {
    std::ofstream os{file.c_str()};
    for (const auto &entry : entries) {
        os << entry.loginName << ":" << entry.encryptedPassword << ":"
           << entry.uid << ":" << entry.gid << ":"
...
std::string PasswdDB::getUsername(uid_t uid) const {
    // Note: the retrieval of a username is usually done through the getpwuid
    // function. However, musl fails to retrieve all the passwd entries from
    // certain systems, e.g. LDAP at CSCS.
    ...
    auto message =
        boost::format("Failed to retrieve username for uid=%d") % uid;
...
boost::filesystem::path PasswdDB::getHomeDirectory(uid_t uid) const {
    for (const auto &entry : entries) {
        if (entry.uid == uid) {
            logMessage(boost::format("Found home directory for uid=%d: %s") %
                           uid % entry.userHomeDirectory,
    ...
    auto message =
        boost::format("Failed to retrieve home directory for uid=%d") % uid;
    SARUS_THROW_ERROR(message.str());
}
...
std::vector<PasswdDB::Entry> &PasswdDB::getEntries() { return entries; }
...
PasswdDB::Entry PasswdDB::parseLine(const std::string &line) const {
    ...
    if (tokens.size() < 6 || tokens.size() > 7) {
        auto message =
            boost::format("Failed to parse line \"%s\": bad number of tokens") %
            line;
        SARUS_THROW_ERROR(message.str());
    }
...
void PasswdDB::logMessage(const boost::format &message,
