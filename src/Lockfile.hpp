 ...
 * The constructor attempts to acquire exclusive access to the shared resource
 * by atomically creating a lock file on the filesystem. If a lock file already
 * exists, i.e. the resource was already acquired by somebody else, the
 * constructor busy waits until the lock file is removed. The destructor
 * releases exclusive access to the shared resource by removing the lock file
 * from the filesystem.
 */
class Lockfile {
  public:
