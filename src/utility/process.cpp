...
std::string executeCommand(const std::string &command) {
  auto commandWithRedirection =
      command + " 2>&1"; // stderr-to-stdout redirection necessary because popen
                         // only reads stdout
  ...
  if (!pipe) {
    auto message =
        boost::format(
            "Failed to execute command \"%s\". Call to popen() failed (%s)") %
  ...
  try {
    ...
    auto message = boost::format("Failed to read stdout from command \"%s\"") %
  ...
  if (status == -1) {
    auto message =
        boost::format(
            "Failed to execute command \"%s\". Call to pclose() failed (%s)") %
        commandWithRedirection % strerror(errno);
    SARUS_THROW_ERROR(message.str());
...
int forkExecWait(
  ...
  if (childStdoutStream) {
    if (pipe(pipefd) == -1) {
      auto message =
          boost::format("Failed to open pipe to execute subprocess %s: %s") %
          args % strerror(errno);
      SARUS_THROW_ERROR(message.str());
    }
  }
  ...
  if (pid == -1) {
    auto message =
        boost::format("Failed to fork to execute subprocess %s: %s") % args %
        strerror(errno);
    SARUS_THROW_ERROR(message.str());
  }
  ...
  if (isChild) {
    if (childStdoutStream) {
      // Redirect stdout to write to the pipe, then we don't need the pipe ends
      // anymore
    ...
    auto message = boost::format("Failed to execvp subprocess %s: %s") % args %
                   strerror(errno);
    SARUS_THROW_ERROR(message.str());
  } else {
    ...
    if (childStdoutStream) {
      ...
      try {
        ...
        auto message =
            boost::format("Failed to read stdout from subprocess %s") % args;
        SARUS_RETHROW_ERROR(e, message.str());
      }
    }
    int status;
    ...
    logMessage(boost::format("%s (pid %d) exited with status %d") % args % pid %
                   WEXITSTATUS(status),
               libsarus::LogLevel::DEBUG);
...
std::vector<int> getCpuAffinity() {
  ...
  if (sched_getaffinity(getpid(), sizeof(cpu_set_t), &set) != 0) {
    auto message =
        boost::format("sched_getaffinity failed: %s ") % std::strerror(errno);
    SARUS_THROW_ERROR(message.str());
  }
...
void setCpuAffinity(const std::vector<int> &cpus) {
  ...
  if (sched_setaffinity(getpid(), sizeof(cpu_set_t), &set) != 0) {
    auto message =
        boost::format{"sched_setaffinity failed: %s"} % std::strerror(errno);
    SARUS_THROW_ERROR(message.str());
  }
