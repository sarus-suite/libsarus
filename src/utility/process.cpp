...
static void logProcessUserAndGroupIdentifiers() {
    ...
    logMessage(boost::format("Current uids (r/e/s/fs): %d %d %d %d") % ruid %
                   euid % suid % setfsuid(-1),
               libsarus::LogLevel::DEBUG);
    logMessage(boost::format("Current gids (r/e/s/fs): %d %d %d %d") % rgid %
                   egid % sgid % setfsgid(-1),
               libsarus::LogLevel::DEBUG);
}
...
void switchIdentity(const libsarus::UserIdentity& identity) {
    ...
    if (euid == 0) {
        ...
        if (setgroups(identity.supplementaryGids.size(),
                      identity.supplementaryGids.data()) != 0) {
 ...
 *
 * However, when having to bind-mount files which reside on root_squashed
 * filesystems, a process needs to have both root privileges (to perform the
 * mount) and normal user filesystem permissions (under root_squash, root is
 * remapped to nobody and cannot access the user content unless said content is
 * world-readable). The above is the main scenario in which this function is
 * meant to be used. Other similar use cases where both root privileges and user
 * permissions are required might occur.
 */
    ...
    while (!feof(in)) {
        if (fgets(buffer, sizeof(buffer), in)) {
            *out << buffer;
            ...
            SARUS_THROW_ERROR(
                "Failed to read C stream: call to fgets() failed.");
        }
    }
}
...
std::string executeCommand(const std::string& command) {
    auto commandWithRedirection =
        command + " 2>&1";  // stderr-to-stdout redirection necessary because
                            // popen only reads stdout
    logMessage(boost::format("Executing command '%s'") % commandWithRedirection,
               libsarus::LogLevel::DEBUG);

    FILE* pipe = popen(commandWithRedirection.c_str(), "r");
    if (!pipe) {
        auto message = boost::format(
                           "Failed to execute command \"%s\". Call to popen() "
                           "failed (%s)") %
    ...
    try {
        ...
        auto message =
            boost::format("Failed to read stdout from command \"%s\"") %
    ...
    if (status == -1) {
        auto message = boost::format(
                           "Failed to execute command \"%s\". Call to pclose() "
                           "failed (%s)") %
                       commandWithRedirection % strerror(errno);
        SARUS_THROW_ERROR(message.str());
...
int forkExecWait(
    ...
    logMessage(boost::format("Forking and executing '%s'") % args,
               libsarus::LogLevel::DEBUG);
    ...
    if (childStdoutStream) {
        if (pipe(pipefd) == -1) {
            auto message =
                boost::format(
                    "Failed to open pipe to execute subprocess %s: %s") %
                args % strerror(errno);
            SARUS_THROW_ERROR(message.str());
        }
    }
    ...
    if (pid == -1) {
        auto message =
            boost::format("Failed to fork to execute subprocess %s: %s") %
            args % strerror(errno);
        SARUS_THROW_ERROR(message.str());
    }
    ...
    if (isChild) {
        if (childStdoutStream) {
            // Redirect stdout to write to the pipe, then we don't need the pipe
            // ends anymore
        ...
        auto message = boost::format("Failed to execvp subprocess %s: %s") %
                       args % strerror(errno);
        SARUS_THROW_ERROR(message.str());
    } else {
        ...
        if (childStdoutStream) {
            ...
            try {
                ...
                auto message =
                    boost::format("Failed to read stdout from subprocess %s") %
                    args;
                SARUS_RETHROW_ERROR(e, message.str());
            }
        }
        ...
        do {
            if (waitpid(pid, &status, 0) == -1) {
                auto message =
                    boost::format("Failed to waitpid subprocess %s: %s") %
                    args % strerror(errno);
                SARUS_THROW_ERROR(message.str());
            }
        ...
        if (!WIFEXITED(status)) {
            auto message =
                boost::format("Subprocess %s terminated abnormally") % args;
            SARUS_THROW_ERROR(message.str());
        }

        logMessage(boost::format("%s (pid %d) exited with status %d") % args %
...
std::string getHostname() {
    ...
    if (gethostname(hostname, HOST_NAME_MAX) != 0) {
        auto message =
            boost::format("failed to retrieve hostname (%s)") % strerror(errno);
        SARUS_THROW_ERROR(message.str());
    }
...
std::vector<int> getCpuAffinity() {
    logMessage("Getting CPU affinity (list of CPU ids)",
               libsarus::LogLevel::INFO);

    auto set = cpu_set_t{};

    if (sched_getaffinity(getpid(), sizeof(cpu_set_t), &set) != 0) {
        auto message = boost::format("sched_getaffinity failed: %s ") %
                       std::strerror(errno);
        SARUS_THROW_ERROR(message.str());
    }
    ...
    for (int cpu = 0; cpu < CPU_SETSIZE; ++cpu) {
        if (CPU_ISSET(cpu, &set)) {
            cpus.push_back(cpu);
            logMessage(boost::format("Detected CPU %d") % cpu,
                       libsarus::LogLevel::DEBUG);
        }
    }
...
void setCpuAffinity(const std::vector<int>& cpus) {
    ...
    for (auto cpu : cpus) {
        CPU_SET(cpu, &set);
        logMessage(boost::format("Set CPU %d") % cpu,
                   libsarus::LogLevel::DEBUG);
    }

    if (sched_setaffinity(getpid(), sizeof(cpu_set_t), &set) != 0) {
        auto message = boost::format{"sched_setaffinity failed: %s"} %
                       std::strerror(errno);
        SARUS_THROW_ERROR(message.str());
    }
