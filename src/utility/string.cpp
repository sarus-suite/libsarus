...
parseKeyValuePair(const std::string &pairString, const char separator) {
...
parseMap(const std::string &input, const char pairSeparators,
  ...
  for (const auto &pair : pairs) {
    ...
    if (valueEnd != value.cend()) {
      auto message =
          boost::format("Error parsing '%s'. Invalid key-value pair '%s': "
    ...
    if (map.find(key) != map.cend()) {
      auto message =
          boost::format("Error parsing '%s'. Found duplicated key '%s': "
