...
rapidjson::Document parse(const std::string &string) {
    ...
    if (json.HasParseError()) {
        auto message = boost::format(
                           ...
                           "not valid JSON\n"
                           "Error(offset %u): %s") %
                       string % static_cast<unsigned>(json.GetErrorOffset()) %
    ...
    if (json.HasParseError()) {
        auto message =
            boost::format(
                ...
                "Error(offset %u): %s") %
            filename % static_cast<unsigned>(json.GetErrorOffset()) %
            rapidjson::GetParseError_En(json.GetParseError());
        SARUS_THROW_ERROR(message.str());
    }
    return json;
...
rapidjson::SchemaDocument readSchema(
    ...
    class RemoteSchemaDocumentProvider
      ...
      public:
        ...
        const rapidjson::SchemaDocument *GetRemoteDocument(
            const char *uri, rapidjson::SizeType length) override {
    ...
    return rapidjson::SchemaDocument{schemaJSON, nullptr,
                                     rapidjson::SizeType(0), &provider};
}
...
rapidjson::Document readAndValidate(const boost::filesystem::path &jsonFile,
    ...
    try {
        ...
        // Parse JSON from reader, validate the SAX events, and populate the
        // configuration Document.
        rapidjson::SchemaValidatingReader<rapidjson::kParseDefaultFlags,
                                          rapidjson::IStreamWrapper,
                                          rapidjson::UTF8<>>
            reader(configStreamWrapper, schema);
        ...
        if (!reader.GetParseResult()) {
            ...
            // (1) the validator found that the JSON is invalid according to
            // schema; or (2) the input stream has I/O error. Check the
            // validation result
            if (!reader.IsValid()) {
                ...
                auto message =
                    boost::format("Error parsing JSON file: %s") % jsonFile;
                SARUS_THROW_ERROR(message.str());
            }
        }
