...
readSchema(const boost::filesystem::path &schemaFile) {
  ...
  return rapidjson::SchemaDocument{schemaJSON, nullptr, rapidjson::SizeType(0),
                                   &provider};
}
...
rapidjson::Document readAndValidate(const boost::filesystem::path &jsonFile,
  ...
  try {
    ...
    if (!reader.GetParseResult()) {
      ...
      // or (2) the input stream has I/O error. Check the validation result
      if (!reader.IsValid()) {
