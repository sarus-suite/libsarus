...
std::pair<std::string, std::string> parseVariable(const std::string &variable) {
  ...
  try {
    ...
    auto message =
        boost::format("Failed to parse environment variable: %s") % e.what();
...
std::string getVariable(const std::string &key) {
  ...
  if ((p = getenv(key.c_str())) == nullptr) {
    auto message =
        boost::format("Environment doesn't contain variable with key %s") % key;
    SARUS_THROW_ERROR(message.str());
  }
  logMessage(boost::format("Got environment variable %s=%s") % key % p,
...
void setVariable(const std::string &key, const std::string &value) {
  ...
  if (setenv(key.c_str(), value.c_str(), overwrite) != 0) {
    auto message = boost::format("Failed to setenv(%s, %s, %d): %s") % key %
                   value % overwrite % strerror(errno);
    SARUS_THROW_ERROR(message.str());
  }
  logMessage(boost::format("Set environment variable %s=%s") % key % value,
             libsarus::LogLevel::DEBUG);
}
