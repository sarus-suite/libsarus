...
static void replaceFd(int oldfd, int newfd) {
    if (dup2(newfd, oldfd) == -1) {
        auto message =
            boost::format("Failed to replace fd with 'dup(%d, %d)': %s") %
            oldfd % newfd % std::strerror(errno);
...
void applyLoggingConfigIfAvailable(const rapidjson::Document& json) {
    ...
    if (json["annotations"].HasMember("com.hooks.logging.stdoutfd")) {
        int fd =
            open(json["annotations"]["com.hooks.logging.stdoutfd"].GetString(),
                 O_WRONLY);
        replaceFd(1, fd);
        close(fd);
    }
    ...
    if (json["annotations"].HasMember("com.hooks.logging.stderrfd")) {
        int fd =
            open(json["annotations"]["com.hooks.logging.stderrfd"].GetString(),
                 O_WRONLY);
        replaceFd(2, fd);
        close(fd);
    }
...
ContainerState parseStateOfContainerFromStdin() {
    try {
        ...
        SARUS_RETHROW_ERROR(
            e, "Failed to parse container's state JSON from stdin.");
    }
}
    ...
    for (const auto& line : mountinfoLines) {
        ...
        if (boost::starts_with(mountRoot, "/..")) {
            auto message = boost::format(
                               "\"%s\" cgroup subsystem mount at %s belongs to "
                               "a parent cgroup namespace") %
...
boost::filesystem::path findCgroupPathInHierarchy(
    ...
    for (const auto& line : procFileLines) {
        ...
        if (boost::starts_with(cgroupPathStr, "/..")) {
            auto message = boost::format(
                               "\"%s\" cgroup hierarchy for process %d is "
        ...
        hook::logMessage(
            boost::format(
                "Found \"%s\" cgroup relative path for process %d: %s") %
    ...
    auto message = boost::format(
                       "Could not find \"%s\" cgroup relative path for process "
                       "%d within %s") %
...
boost::filesystem::path findCgroupPath(
    ...
    if (!boost::filesystem::exists(cgroupPath)) {
        auto message = boost::format(
                           "Found cgroups \"%s\" subsystem for process %d in "
                           "%s, but directory doesn't exist") %
                       subsystemName % pid % cgroupPath;
        SARUS_THROW_ERROR(message.str());
    }
...
void switchToUnprivilegedProcess(const uid_t targetUid, const gid_t targetGid) {
    ...
    for (int capIdx = 0;; capIdx++) {
        if (prctl(PR_CAPBSET_DROP, capIdx, 0, 0, 0) != 0) {
            if (errno == EINVAL) {
                ...
                auto message =
                    boost::format(
                        "Failed to prctl(PR_CAPBSET_DROP, %d, 0, 0, 0): %s") %
    ...
    if (setresgid(targetGid, targetGid, targetGid) != 0) {
        auto message =
            boost::format("Failed to setresgid(%1%, %1%, %1%): %2%") %
    ...
    if (setresuid(targetUid, targetUid, targetUid) != 0) {
        auto message =
            boost::format("Failed to setresuid(%1%, %1%, %1%): %2%") %
    ...
    if (prctl(PR_SET_NO_NEW_PRIVS, 1, 0, 0, 0) != 0) {
        auto message =
            boost::format(
                "Failed to prctl(PR_SET_NO_NEW_PRIVS, 1, 0, 0, 0): %s") %
            strerror(errno);
        SARUS_THROW_ERROR(message.str());
    }
}

std::tuple<unsigned int, unsigned int> parseLibcVersionFromLddOutput(
    const std::string& lddOutput) {
