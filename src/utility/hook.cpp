...
std::string ContainerState::status() const {
  return state["status"].GetString();
};
...
static void replaceFd(int oldfd, int newfd) {
  if (dup2(newfd, oldfd) == -1) {
    auto message =
        boost::format("Failed to replace fd with 'dup(%d, %d)': %s") % oldfd %
        newfd % std::strerror(errno);
...
void applyLoggingConfigIfAvailable(const rapidjson::Document &json) {
  ...
  if (json["annotations"].HasMember("com.hooks.logging.stdoutfd")) {
    int fd = open(json["annotations"]["com.hooks.logging.stdoutfd"].GetString(),
                  O_WRONLY);
    replaceFd(1, fd);
    close(fd);
  }
  ...
  if (json["annotations"].HasMember("com.hooks.logging.stderrfd")) {
    int fd = open(json["annotations"]["com.hooks.logging.stderrfd"].GetString(),
                  O_WRONLY);
    replaceFd(2, fd);
    close(fd);
  }
...
ContainerState parseStateOfContainerFromStdin() {
  try {
    ...
    SARUS_RETHROW_ERROR(e,
                        "Failed to parse container's state JSON from stdin.");
  }
}

std::unordered_map<std::string, std::string>
parseEnvironmentVariablesFromOCIBundle(
...
boost::optional<std::string> getEnvironmentVariableValueFromOCIBundle(
    const std::string &key, const boost::filesystem::path &bundleDir) {
...
static void enterNamespace(const boost::filesystem::path &namespaceFile) {
  ...
  if (fd == -1) {
    auto message = boost::format("Failed to open namespace file %s: %s") %
                   namespaceFile % strerror(errno);
    SARUS_THROW_ERROR(message.str());
  }

  // enter namespace
  if (setns(fd, 0) != 0) {
    auto message = boost::format("Failed to enter namespace %s: %s") %
                   namespaceFile % strerror(errno);
    SARUS_THROW_ERROR(message.str());
  }
}
...
/**
 * Find the mount root and mount point of a cgroup subsystem by parsing the
 * [procPrefixDir]/proc/[pid]/mountinfo file. For details about the syntax of
...
findSubsystemMountPaths(const std::string &subsystemName,
  ...
  hook::logMessage(
      boost::format("Parsing %s for \"%s\" cgroup subsystem mount paths") %
          mountinfoPath % subsystemName,
  ...
  for (const auto &line : mountinfoLines) {
    ...
    if (mountRoot.empty() || mountPoint.empty() || filesystemType.empty() ||
        superOptions.empty()) {
      continue;
    }
    ...
    if (boost::starts_with(mountRoot, "/..")) {
      auto message = boost::format("\"%s\" cgroup subsystem mount at %s "
                                   "belongs to a parent cgroup namespace") %
                     subsystemName % mountPoint;
      SARUS_THROW_ERROR(message.str());
    }

    hook::logMessage(
        boost::format("Found \"%s\" cgroup subsystem mount root: %s") %
            subsystemName % mountRoot,
        libsarus::LogLevel::DEBUG);
    hook::logMessage(
        boost::format("Found \"%s\" cgroup subsystem mount point: %s") %
            subsystemName % mountPoint,
        libsarus::LogLevel::DEBUG);
    return std::tuple<boost::filesystem::path, boost::filesystem::path>{
        mountRoot, mountPoint};
  }

  auto message =
      boost::format(
          "Could not find \"%s\" cgroup subsystem mount point within %s") %
      subsystemName % mountinfoPath;
  SARUS_THROW_ERROR(message.str());
}

/**
 * Find the pathname of a given control group to which a process belongs by
 * parsing the [procPrefixDir]/proc/[pid]/cgroup file. For details about the
 * syntax of such file, please refer to the cgroups(7) man page. For details
...
findCgroupPathInHierarchy(const std::string &subsystemName,
                          const boost::filesystem::path &procPrefixDir,
                          const boost::filesystem::path &subsystemMountRoot,
                          const pid_t pid) {
  ...
  hook::logMessage(boost::format("Parsing %s for \"%s\" cgroup path relative "
                                 "to hierarchy mount point") %
                       procFilePath % subsystemName,
                   libsarus::LogLevel::DEBUG);
  ...
  for (const auto &line : procFileLines) {
    ...
    if (boost::starts_with(cgroupPathStr, "/..")) {
      auto message = boost::format("\"%s\" cgroup hierarchy for process %d is "
                                   "rooted in another cgroup namespace") %
                     subsystemName % pid;
      SARUS_THROW_ERROR(message.str());
    }
    if (subsystemMountRoot.string() != "/" &&
        boost::starts_with(cgroupPathStr, subsystemMountRoot.string())) {
      cgroupPath /=
          boost::filesystem::relative(cgroupPathStr, subsystemMountRoot);
    } else {
      cgroupPath = boost::filesystem::path(cgroupPathStr);
    }

    hook::logMessage(
        boost::format("Found \"%s\" cgroup relative path for process %d: %s") %
            subsystemName % pid % cgroupPath,
        libsarus::LogLevel::DEBUG);
    return boost::filesystem::path(cgroupPath);
  }

  auto message = boost::format("Could not find \"%s\" cgroup relative path for "
                               "process %d within %s") %
...
boost::filesystem::path
findCgroupPath(const std::string &subsystemName,
               const boost::filesystem::path &procPrefixDir, const pid_t pid) {
  hook::logMessage(
      boost::format(
          "Searching for cgroup \"%s\" subsystem under %s for process %d") %
          subsystemName % procPrefixDir % pid,
      libsarus::LogLevel::DEBUG);

  boost::filesystem::path subsystemMountRoot;
  boost::filesystem::path subsystemMountPoint;
  std::tie(subsystemMountRoot, subsystemMountPoint) =
      findSubsystemMountPaths(subsystemName, procPrefixDir, pid);
  auto cgroupRelativePath = findCgroupPathInHierarchy(
      subsystemName, procPrefixDir, subsystemMountRoot, pid);

  auto cgroupPath = subsystemMountPoint / cgroupRelativePath;

  if (!boost::filesystem::exists(cgroupPath)) {
    auto message = boost::format("Found cgroups \"%s\" subsystem for process "
                                 "%d in %s, but directory doesn't exist") %
                   subsystemName % pid % cgroupPath;
    SARUS_THROW_ERROR(message.str());
  }

  hook::logMessage(
      boost::format("Found cgroups \"%s\" subsystem for process %d in %s") %
          subsystemName % pid % cgroupPath,
      libsarus::LogLevel::DEBUG);
  return cgroupPath;
}
...
void whitelistDeviceInCgroup(const boost::filesystem::path &cgroupPath,
  ...
  try {
    ...
    auto message =
        boost::format("Failed to whitelist %s: not a valid device file") %
        deviceFile;
    SARUS_RETHROW_ERROR(e, message.str());
  }

  auto deviceID = filesystem::getDeviceID(deviceFile);
  auto entry = boost::format("%c %u:%u rw") % deviceType % major(deviceID) %
               minor(deviceID);
  hook::logMessage(boost::format("Whitelist entry: %s") % entry.str(),
                   libsarus::LogLevel::DEBUG);
  ...
  hook::logMessage(
      boost::format("Successfully whitelisted device %s for rw access") %
          deviceFile,
      libsarus::LogLevel::DEBUG);
}
...
void switchToUnprivilegedProcess(const uid_t targetUid, const gid_t targetGid) {
  ...
  for (int capIdx = 0;; capIdx++) {
    if (prctl(PR_CAPBSET_DROP, capIdx, 0, 0, 0) != 0) {
      if (errno == EINVAL) {
        ...
        auto message =
            boost::format("Failed to prctl(PR_CAPBSET_DROP, %d, 0, 0, 0): %s") %
  ...
  if (setgroups(0, NULL) != 0) {
    auto message =
        boost::format("Failed to setgroups(0, NULL): %s") % strerror(errno);
    SARUS_THROW_ERROR(message.str());
  }

  // change to user's gid
  if (setresgid(targetGid, targetGid, targetGid) != 0) {
    auto message = boost::format("Failed to setresgid(%1%, %1%, %1%): %2%") %
  ...
  if (setresuid(targetUid, targetUid, targetUid) != 0) {
    auto message = boost::format("Failed to setresuid(%1%, %1%, %1%): %2%") %
  ...
  if (prctl(PR_SET_NO_NEW_PRIVS, 1, 0, 0, 0) != 0) {
    auto message =
        boost::format("Failed to prctl(PR_SET_NO_NEW_PRIVS, 1, 0, 0, 0): %s") %
        strerror(errno);
    SARUS_THROW_ERROR(message.str());
  }
}
...
parseLibcVersionFromLddOutput(const std::string &lddOutput) {
  ...
  if (boost::regex_match(outputLines[0].c_str(), matches, re)) {
    return std::tuple<unsigned int, unsigned int>{std::stoi(matches[1]),
                                                  std::stoi(matches[2])};
  } else {
    auto message =
        boost::format(
            "Failed to parse glibc version from ldd output head:\n%s") %
        outputLines[0];
    SARUS_THROW_ERROR(message.str());
  }
}
