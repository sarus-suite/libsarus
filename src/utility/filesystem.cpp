...
size_t getFileSize(const boost::filesystem::path &filename) {
  ...
  if (stat(filename.c_str(), &st) != 0) {
    auto message =
        boost::format("Failed to retrieve size of file %s. Stat failed: %s") %
...
std::tuple<uid_t, gid_t> getOwner(const boost::filesystem::path &path) {
  ...
  if (stat(path.c_str(), &sb) != 0) {
    auto message =
        boost::format("Failed to retrieve owner of file %s. Stat failed: %s") %
        path % strerror(errno);
    SARUS_THROW_ERROR(message.str());
  }
...
void setOwner(const boost::filesystem::path &path, uid_t uid, gid_t gid) {
  if (uid == static_cast<uid_t>(-1) || gid == static_cast<uid_t>(-1)) {
    assert(uid == static_cast<uid_t>(-1) && gid == static_cast<gid_t>(-1));
    return;
  }

  if (!boost::filesystem::exists(path)) {
    auto message =
        boost::format("attempted to change ownership of non existing path %s") %
        path;
    SARUS_THROW_ERROR(message.str());
  }
  ...
  if (chown(path.c_str(), uid, gid) != 0) {
    auto errorMessage =
        boost::format("failed to change ownership of path: %s") % path;
  }
}

void createFoldersIfNecessary(const boost::filesystem::path &path, uid_t uid,
                              gid_t gid) {
  ...
  for (const auto &element : path) {
    ...
    if (!boost::filesystem::exists(currentPath)) {
      ...
      try {
        ...
        auto message =
            boost::format("Failed to create directory %s") % currentPath;
...
void createFileIfNecessary(const boost::filesystem::path &path, uid_t uid,
                           gid_t gid) {
  // NOTE: Broken symlinks will NOT be recognized as existing and hence will be
  // overridden.
  if (boost::filesystem::exists(path)) {
...
void copyFile(const boost::filesystem::path &src,
              const boost::filesystem::path &dst, uid_t uid, gid_t gid) {
...
void copyFolder(const boost::filesystem::path &src,
                const boost::filesystem::path &dst, uid_t uid, gid_t gid) {
  if (!boost::filesystem::exists(src) ||
      !boost::filesystem::is_directory(src)) {
    auto message =
        boost::format("Failed to copy %s to %s: source folder doesn't exist.") %
        src % dst;
    SARUS_THROW_ERROR(message.str());
  }
  ...
  if (boost::filesystem::exists(dst)) {
    auto message =
        boost::format("Failed to copy %s to %s: destination already exists.") %
        src % dst;
    SARUS_THROW_ERROR(message.str());
  }
...
void changeDirectory(const boost::filesystem::path &path) {
  if (!boost::filesystem::exists(path)) {
    auto message =
        boost::format("attemped to cd into %s, but directory doesn't exist") %
        path;
    SARUS_THROW_ERROR(message.str());
  }
  ...
  if (chdir(path.string().c_str()) != 0) {
    auto message =
        boost::format("failed to cd into %s: %s") % path % strerror(errno);
    SARUS_THROW_ERROR(message.str());
  }
}
...
int countFilesInDirectory(const boost::filesystem::path &path) {
  if (!boost::filesystem::exists(path) ||
    ...
    auto message =
        boost::format(
            "Failed to count files in %s: path is not an existing directory.") %
        path;
    SARUS_THROW_ERROR(message.str());
  }
  ...
  // The directory_iterator() ctor without arguments always creates the "end"
  // iterator
  auto numberOfFiles = static_cast<int>(
      std::distance(path_iterator, boost::filesystem::directory_iterator()));
...
void writeTextFile(const std::string &text,
  ...
  try {
    ...
    if (!ofs) {
      auto message =
          boost::format("Failed to open std::ofstream for %s") % filename;
      SARUS_THROW_ERROR(message.str());
    }
    ofs << text;
  } catch (const std::exception &e) {
...
/**
 * Generates a random suffix and append it to the given path. If the generated
 * random path exists, tries again with another suffix until the operation
 * succeedes.
 *
 * Note: boost::filesystem::unique_path offers a similar functionality. However,
 * it fails (throws exception) when the locale configuration is invalid. More
 * specifically, we experienced the problem when LC_CTYPE was set to UTF-8 and
 * the locale UTF-8 was not installed.
 */
boost::filesystem::path
makeUniquePathWithRandomSuffix(const boost::filesystem::path &path) {
  auto uniquePath = std::string{};

  do {
    const size_t sizeOfRandomSuffix = 16;
    uniquePath =
        path.string() + "-" + string::generateRandom(sizeOfRandomSuffix);
...
getSymlinkTarget(const boost::filesystem::path &path) {
  char buffer[PATH_MAX];
  auto count = readlink(path.string().c_str(), buffer, PATH_MAX);
  assert(count < PATH_MAX); // PATH_MAX is supposed to be large enough for any
                            // path + NULL terminating char
  buffer[count] = '\0';
  return buffer;
}
...
/*
    ...
    At the end of the function execution, the optional output parameter
   'traversedSymlinks' contains the various symlinks that were traversed during
  ...
  for (const auto &element : path1) {
    if (element == "/") {
      ...
      if (target.is_absolute()) {
        current =
            appendPathsWithinRootfs(rootfs, "/", target, traversedSymlinks);
      } else {
        current =
            appendPathsWithinRootfs(rootfs, current, target, traversedSymlinks);
      }
    } else {
      current /= element;
    }
...
realpathWithinRootfs(const boost::filesystem::path &rootfs,
  ...
  if (!path.is_absolute()) {
    auto message = boost::format("Failed to determine realpath within rootfs. "
...
dev_t getDeviceID(const boost::filesystem::path &path) {
  ...
  if (stat(path.c_str(), &sb) != 0) {
    auto message =
        boost::format(
            "Failed to retrieve device ID of file %s. Stat failed: %s") %
        path % strerror(errno);
    SARUS_THROW_ERROR(message.str());
  }
  logMessage(boost::format("Got device ID for %s: %d") % path % sb.st_rdev,
             LogLevel::DEBUG);
  return sb.st_rdev;
}
...
char getDeviceType(const boost::filesystem::path &path) {
  ...
  if (filesystem::isCharacterDevice(path)) {
    ...
    auto message =
        boost::format("Failed to recognize device type of file %s."
                      " File is not a device or has unknown device type.") %
        path;
    SARUS_THROW_ERROR(message.str());
  }
  logMessage(boost::format("Got device type for %s: '%c'") % path % deviceType,
             LogLevel::DEBUG);
  return deviceType;
}
...
bool isDeviceFile(const boost::filesystem::path &path) {
  ...
  if (stat(path.c_str(), &sb) != 0) {
    auto message =
        boost::format(
            "Failed to check if file %s is a device file. Stat failed: %s") %
        path % strerror(errno);
    SARUS_THROW_ERROR(message.str());
  }
...
bool isBlockDevice(const boost::filesystem::path &path) {
  ...
  if (stat(path.c_str(), &sb) != 0) {
    auto message =
        boost::format(
            "Failed to check if file %s is a block device. Stat failed: %s") %
        path % strerror(errno);
    SARUS_THROW_ERROR(message.str());
  }
...
bool isCharacterDevice(const boost::filesystem::path &path) {
  struct stat sb;
  if (stat(path.c_str(), &sb) != 0) {
    auto message = boost::format("Failed to check if file %s is a charater "
                                 "device. Stat failed: %s") %
                   path % strerror(errno);
    SARUS_THROW_ERROR(message.str());
  }
...
bool isSharedLib(const boost::filesystem::path &file) {
  ...
  auto pos = std::find_end(filename.cbegin(), filename.cend(),
                           extension.cbegin(), extension.cend());
