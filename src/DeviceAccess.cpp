...
void DeviceAccess::parseInput(const std::string &input) {
  ...
  if (input.size() > 3) {
    auto message =
        boost::format(
            "Input string for device access '%s' is longer than 3 characters") %
        input;
  ...
  for (const char &c : input) {
    if (c == 'r') {
      if (isReadAllowed()) {
        auto message =
            boost::format(
                "Input string for device access '%s' has repeated characters") %
            input;
        SARUS_THROW_ERROR(message.str());
      }
      allowRead();
      ...
      auto message = boost::format("Input string for device access '%s' "
