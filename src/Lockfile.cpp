...
Lockfile::Lockfile() : logger(&libsarus::Logger::getInstance()) {}

Lockfile::Lockfile(const boost::filesystem::path &file, unsigned int timeoutMs,
                   unsigned int warningMs)
    : logger(&libsarus::Logger::getInstance()), lockfile{
                                                    convertToLockfile(file)} {
  ...
  while (!createLockfileAtomically()) {
    if (timeoutMs != noTimeout && elapsedTimeMs >= timeoutMs) {
      message = boost::format("Failed to acquire lock on file %s (expired "
...
Lockfile::Lockfile(Lockfile &&rhs)
    : logger{rhs.logger}, lockfile{rhs.lockfile} {
...
Lockfile::convertToLockfile(const boost::filesystem::path &file) const {
  auto lockfile = file;
  lockfile += ".lock";

  auto message =
      boost::format("converted filename %s to lockfile %s") % file % lockfile;
  logger->log(message.str(), loggerSubsystemName, libsarus::LogLevel::DEBUG);
...
bool Lockfile::createLockfileAtomically() const {
  ...
  logger->log("successfully created lockfile", loggerSubsystemName,
              libsarus::LogLevel::DEBUG);

  if (close(fd) != 0) {
    message = boost::format("failed to close file descriptor of lockfile %s") %
              *lockfile;
    SARUS_THROW_ERROR(message.str());
  }
  return true;
