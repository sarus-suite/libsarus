...
/**
 * This class encapsulates error trace information to be propagated as an
 * exception.
 *
 * An error trace entry encapsulates information about file, line and function
 * name where the error trace entry was created.
 *
 ...
 *
 * Note: this class should be instantiated and thrown through the
 * SARUS_THROW_ERROR macro. Caught instances of this class should be rethrown
 * through the SARUS_RETHROW_ERROR macro. The user is not supposed to
 * instantiate and throw this class "manually".
 */
...
public:
  Error(LogLevel logLevel, const ErrorTraceEntry &entry)
      : logLevel{logLevel}, errorTrace{entry} {}

  const char *what() const noexcept override {
    // Return the 'what()' of the original exception that generated this error
    // trace as if the original exception was propagated directly up to the
    // current stack frame, i.e. without intermediate catch-rethrows.
...
inline bool operator==(const Error::ErrorTraceEntry &lhs,
                       const Error::ErrorTraceEntry &rhs) {
  return lhs.errorMessage == rhs.errorMessage && lhs.fileName == rhs.fileName &&
         lhs.fileLine == rhs.fileLine && lhs.functionName == rhs.functionName;
}
...
#define SARUS_THROW_ERROR_2(errorMessage, logLevel)                            \
  {                                                                            \
    auto stackTraceEntry = libsarus::Error::ErrorTraceEntry{                   \
        errorMessage, __FILENAME__, __LINE__, __func__};                       \
    throw libsarus::Error{logLevel, stackTraceEntry};                          \
  }

#define SARUS_THROW_ERROR_1(errorMessage)                                      \
  SARUS_THROW_ERROR_2(errorMessage, libsarus::LogLevel::ERROR)

#define SARUS_THROW_ERROR(...)                                                 \
  SARUS_GET_OVERLOADED_THROW_ERROR(__VA_ARGS__, SARUS_THROW_ERROR_2,           \
                                   SARUS_THROW_ERROR_1)                        \
  (__VA_ARGS__)
...
#define SARUS_RETHROW_ERROR_3(exception, errorMessage, logLevel)               \
  {                                                                            \
    ...
    if (cp) { /* check if dynamic type is libsarus::Error */                   \
      assert(!std::is_const<                                                   \
             decltype(exception)>{}); /* a libsarus::Error object must be      \
                                         caught as non-const reference because \
                                         we need to modify its internal error  \
                                         trace */                              \
...
#define SARUS_RETHROW_ERROR_2(exception, errorMessage)                         \
  {                                                                            \
    ...
    if (cp) {                                                                  \
      ...
      SARUS_RETHROW_ERROR_3(exception, errorMessage,                           \
                            libsarus::LogLevel::ERROR)                         \
    }                                                                          \
  }

#define SARUS_RETHROW_ERROR(...)                                               \
  SARUS_GET_OVERLOADED_RETHROW_ERROR(__VA_ARGS__, SARUS_RETHROW_ERROR_3,       \
                                     SARUS_RETHROW_ERROR_2)                    \
  (__VA_ARGS__)
